...
if [ -n "$PRIMARY" ] && [ "$PRIMARY" != "$PORTRAIT_MONITOR" ] && xrandr | grep -q "^${PORTRAIT_MONITOR} connected"; then
  ...
  if [ -n "$PORTRAIT_MODE" ]; then
    xrandr --output "$PORTRAIT_MONITOR" --mode "$PORTRAIT_MODE" --rotate right --right-of "$PRIMARY" --scale 1x1
  else
    xrandr --output "$PORTRAIT_MONITOR" --auto --rotate right --right-of "$PRIMARY" --scale 1x1
  fi
fi
